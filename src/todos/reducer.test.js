...
test('adds toggled todo to list', () => {
  let state = reducer(undefined, a.addTodo(todo1));
  const toggledTodo = { ...todo1, done: !todo1.done };
  state = reducer(state, a.toggleTodo(toggledTodo));
  expect(Object.keys(state).length).toBe(1);
  expect(state).toEqual({
...
test('removes todo', () => {
  let state = reducer(undefined, a.addTodo(todo1));
  state = reducer(state, a.removeTodo(todo1));
